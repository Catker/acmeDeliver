...
4. **证书校验** - 校验 key.pem 与 cert.pem / fullchain.pem 可配对（`--dry-run` 同样校验），不通过则不写任何文件、不重载
5. **安全部署** - 同目录临时文件 + 原子替换写入目标位置；新建文件 key.pem 权限 0600、其余 0644，已存在的文件保留原权限与属主，部署目标的软链接保留并写入其真实文件（悬空软链接报错）；工作目录写入不跟随软链接（预置软链接会被替换为普通文件）；部署成功后才更新工作目录 time.log
6. **执行重载** - 运行 `reloadcmd` 命令（批量去重），带 15 秒超时控制
